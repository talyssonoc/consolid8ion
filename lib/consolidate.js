 ...
 * consolidate
 *
 * @author Talysson Oliveira Cassiano <talyssonoc@gmail.com>
 * @author originally by TJ Holowaychuk <tj@vision-media.ca>
...
//Caches the requires, so the programmer can send libs
//with pre-settings
exports = function consolidate(requireCache, options) {

  exports.options = options || {};

  if(typeof requireCache === 'object') {
...
exports.getCache = function(engine) {
  if(requires[engine]) {
    return requires[engine];
  }
  else {
    try {
      exports[engine]();
...
exports.nunjucks.render = function(str, options, fn) {
  try {
    ...
    if (loader) {
        ...
        if(exports.options.i18n) {
          env.addExtension('I18nExtension', new engine.i18nExtension());
        }
